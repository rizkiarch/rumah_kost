...
trait WatsappTrait
{
    ...
    public static function sendMessages_Starsender($phone, $message)
    {
        ...
        if (curl_errno($curl)) {
            ...
            curl_close($curl);
            \Log::info("Gagal Mengirim Menggunakan Starsender " . date('Y-m-d H:i:s'));
            return 'cURL Error: ' . $error_msg;
        }
        curl_close($curl);
        \Log::info("Berhasil Mengirim Menggunakan Starsender " . date('Y-m-d H:i:s'));
    ...
    public static function sendTextWatsapp__($phone, $message)
    {
        // Membersihkan dan memformat nomor telepon
        $phone = preg_replace('/[^0-9]/', '', $phone);
        $phone = (str_starts_with($phone, '0')) ? '62' . substr($phone, 1) : $phone;
